...
export default function WishlistPage() {
  ...
  // Updated to use new async functions and loading states
  const { 
    wishlistItems, 
    removeFromWishlist, 
    clearWishlist, 
  ...
  useEffect(() => {
    if (typeof window !== 'undefined') {
      ...
      if (message === 'account_not_found') {
        ...
        alert('Authentication required. Please sign in to access your wishlist.');
      }
    }
  }, []);

  // Breadcrumbs for wishlist page
  const breadcrumbs = [
    { name: "Home", href: "/" },
    { name: "Wishlist", href: "#" }
  ];
  ...
  // 🔄 LOADING STATE - Show spinner while checking auth or loading wishlist
      ...
      <React.Fragment key={index}>
        ...
        >
          ...
          <div className="flex justify-between items-center mb-8 mt-4">
            <div>
              <h1 className="text-3xl font-bold text-gray-900">Wishlist ({wishlistItems.length})</h1>
            </div>
          ...
          {/* Wishlist Items Grid - Same layout as shop */}
            ...
            {wishlistItems.map((item) => (
              <div
                ...
                style={{
                  ...
                  margin: 0,
                  width: '280px',
                  height: '420px', // Same height as ProductCard
                  backgroundColor: '#fff',
                  padding: '10px',
              ...
              >
                ...
                <div style={{
                  ...
                  {/* Remove from wishlist X button - top right */}
                  <button
                  ...
                  >
                    <svg width="18" height="20" viewBox="0 0 18 20" fill="none" xmlns="http://www.w3.org/2000/svg">
                      <path d="M17 3.71322H2.33333L3.66667 18.3167H14.3333L15.6667 3.71322H1M9 7.36409V14.6658M12.3333 7.36409L11.6667 14.6658M5.66667 7.36409L6.33333 14.6658M6.33333 3.71322L7 1.2793H11L11.6667 3.71322" stroke="#1E1E1E" strokeWidth="1.56" strokeLinecap="round" strokeLinejoin="round" />
                    </svg>
                  </button>

                  {/* Badge - Same as ProductCard */}
                  {(!item.inStock || (item.originalPrice && item.originalPrice > item.amount)) && (
                    <span style={{
                      position: 'absolute',
                      top: '10px',
                      left: '10px',
                      ...
                      {!item.inStock ? 'Out of Stock' :
                        (item.originalPrice && item.originalPrice > item.amount) ? 'Sale' : ''}
                    </span>
                  )}
                </div>
                ...
                <div style={{
                  ...
                  <div>
                    ...
                    {/* Quantity and Add to Cart Section - Same as ProductCard */}
                    <div style={{
                      display: 'flex',
                      alignItems: 'center',
                      gap: '1rem',
                      ...
                      {/* Quantity Box - Same as ProductCard */}
                      <div style={{
                        display: 'flex',
                        alignItems: 'center'
                      }}>
                      ...
                      <button
                        ...
                        style={{
                          flex: 1,
                          padding: '12px',
                          border: `1px solid ${item.inStock ? '#FF0000' : '#9CA3AF'}`,
          ...
          {wishlistItems.length > 0 && (
            <div className="mt-12 text-center">
              <div className="flex flex-col sm:flex-row gap-4 justify-center items-center">
                ...
                <button
                  onClick={async () => {
                    // Add all in-stock items to cart with their selected quantities
                    let totalItems = 0;
                    wishlistItems.forEach(item => {
                      if (item.inStock) {
                        const quantity = quantities[item.id] || 1;
                        // Add each item with its selected quantity
                        for (let i = 0; i < quantity; i++) {
                          addToCart({
                            id: item.id,
                            itemName: item.itemName,
