...
// ✅ ADD: Helper function to calculate totals with shipping and tax
...
export async function POST(request: NextRequest) {
  console.log('🚀 Bank transfer API called - WITH SHIPPING & TAX');
  ...
  try {
    ...
    const { cart_items, customer_data, bank_details }: {
      cart_items: CartItem[];
    ...
    if (!bank_details?.accountName || !bank_details?.accountNumber || !bank_details?.bankName) {
      return NextResponse.json({
        success: false,
    ...
    // ✅ FIX: Calculate totals WITH shipping and tax
    ...
    console.log('🆔 Order ID generated:', orderId, 'Final Amount:', orderTotals.finalTotal);
    ...
    const docRef = await addDoc(collection(db, 'bankTransferOrders'), orderData);
    ...
    try {
      ...
      const adminResult = await EmailService.sendAdminNotification(emailOrderData);
      adminEmailSent = adminResult.success;
      console.log('📧 Admin notification:', adminEmailSent ? '✅ Sent' : '❌ Failed');
    ...
    try {
      ...
      const customerResult = await EmailService.sendBankTransferInstructions(customerInstructionsData);
      customerEmailSent = customerResult.success;
      console.log('📧 Customer instructions:', customerEmailSent ? '✅ Sent' : '❌ Failed');
      
    } catch (emailError) {
      console.error('❌ Failed to send customer instructions:', emailError);
    }

    console.log('🎉 Bank transfer order created successfully - WITH PROPER TOTALS!');
    ...
    return NextResponse.json({
      ...
      data: {
        ...
        paymentInstructions: {
          ...
          message: 'Transfer the exact amount to the provided bank account, then submit your transaction reference for instant verification.',
          bankDetails: bank_details,
          amount: orderTotals.finalTotal, // ✅ Final total for instructions
          reference: orderId,
          nextStep: 'After transfer, click "I\'ve Completed Transfer" to submit your transaction reference.',
          breakdown: {
            subtotal: orderTotals.subtotal,
      ...
      message: 'Bank transfer order created successfully - complete payment and submit reference for verification'
    });
...
export async function GET(request: NextRequest) {
  try {
    const instructions = {
      bankDetails: {
        accountName: process.env.BANK_ACCOUNT_NAME || 'Symbol Global Services Ltd',
      ...
      processingTime: 'Instant verification for most transactions, manual verification within 2-4 hours',
      supportedBanks: [
        'All Nigerian banks',
        'USSD transfers', 
        'Mobile banking apps',
