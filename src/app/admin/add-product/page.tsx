...
import { db } from "@/lib/firebase"; // Only need db for Firestore
...
export default function SellItemsPage() {
  ...
  const categorySubItems: { [key: string]: string[] } = {
    ...
    "Freezers": ["Chest Freezer", "Upright Freezer", "Mini Freezer", "Deep Freezer"],
    "Microwave": ["Solo Microwave", "Grill Microwave", "Convection Microwave", "Built-in Microwave"],
    "Air Conditioner": ["Split AC", "Window AC", "Portable AC", "Central AC", "Cassette AC"],
  ...
  // Simple compression that actually works
  ...
  const uploadToCloudinary = async (image: File): Promise<string> => {
    ...
    formData.append('upload_preset', process.env.NEXT_PUBLIC_CLOUDINARY_UPLOAD_PRESET!); // You need to set this
    formData.append('cloud_name', process.env.NEXT_PUBLIC_CLOUDINARY_CLOUD_NAME!); // You need to set this
    ...
    try {
      ...
      console.log(`✅ Successfully uploaded ${image.name} to Cloudinary`);
  ...
  const handleImageChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    const selected = Array.from(files).slice(0, 3); // Take only first 3
    
    console.log(`Selected ${selected.length} files for upload`);
    ...
    try {
      ...
      for (let i = 0; i < images.length; i++) {
        ...
        try {
          ...
          console.log(`Skipping image ${i + 1}`);
        }
      }
      ...
      console.log(`Cloudinary upload complete: ${imageURLs.length}/${images.length} images successful`);
      ...
      const itemData = {
        ...
        images: imageURLs, // All images from Cloudinary
