// src/app/shop/categories/[category]/page.tsx - Category with Filters
"use client";
  ...
  useEffect(() => {
    ...
    const fetchProducts = async () => {
      try {
        ...
        const subcategories = [...new Set(productsData.map(p => p.subcategory))].sort();
                ...
                <div className="mb-6">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {availableSubcategories.map((subcategory) => (
                      <Link
                        key={subcategory}
                        href={`/shop/categories/${resolvedParams.category}/${subcategory.toLowerCase().replace(/\s+/g, '-')}`}
                        className="bg-white border border-gray-200 px-3 py-2 rounded-lg text-sm hover:border-red-300 hover:text-red-600 transition-colors"
                      >
                        {subcategory}
                      </Link>
                    ))}
